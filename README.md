...
Pages or any static host unchanged.

### HTTP headers are currently unset

`netlify.toml` used to carry them and has been deleted along with Netlify. They
were never applied on Cloudflare, which reads a `_headers` file from the assets
directory instead. Nothing supplies one yet, so these are all missing in
production:

- `/_next/static/*` - `public, max-age=31536000, immutable`. Content-hashed, so
  the long cache is free.
- `/media/*` - `public, max-age=604800, must-revalidate`. Regenerated by hand.
- `/*` - `X-Content-Type-Options: nosniff`,
  `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: SAMEORIGIN`.

To restore them, put a `_headers` file in `public/` with the same rules in
Cloudflare's syntax. It copies into `out/` on build and is picked up from there.
